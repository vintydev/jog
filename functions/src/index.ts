...
export const sendQuestionnaireSetReminder = functions.pubsub
  ...
  .onRun(async () => {
    try {
      ...
      await Promise.all(userStatsSnapshot.docs.map(async (docSnap) => {
        ...
        const questionnaireTimeSet = data?.symptomStats?.questionnaireTimeSet;
        ...
        if (questionnaireTimeSet === true) {
          ...
          await updateNotificationInteractionRate(userId, "questionnaireSet");
        }
      }
      ));
...
export const sendHourlyJogCreation = functions.pubsub
  .schedule("every hour")
  .timeZone("Europe/London")
  .onRun(async () => {
    const now = new Date();
    ...
    if (currentHour === 12 || currentHour === 15 || currentHour === 17) {
      ...
      for (const doc of usersSnapshot.docs) {
        ...
        if (!userData?.expoPushToken || userData?.isFocusMode) continue;
      ...
      for (const message of messages) {
        await sendNotification(message);
        await updateNotificationInteractionRate(message.data.userId, "jogCreationReminder");
...
export const markMissedReminders = functions.pubsub
  ...
  .onRun(async () => {
    ...
    try {
      ...
      const userMissedCounts: Record<string, number> = {};
...
export const updateReminderStatus = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    if (!reminder || !previousReminder) return null;
    ...
    if (reminder.completeStatus !== newStatus) {
      try {
        ...
        if (previousReminder.completeStatus && previousReminder.userId === reminder.userId) {
          statFieldToIncrement[previousReminder.completeStatus as keyof typeof statFieldToIncrement] = -1;
        }
...
export const updateIncompleteSteps = functions.firestore
  .document("reminders/{reminderId}")
  .onUpdate(async (change, context) => {
...
export const updateIfOverdue = functions.pubsub
...
export const sendTaskReminder = functions.pubsub
  .schedule("every 1 minutes")
  .timeZone("Europe/London")
  .onRun(async () => {
    ...
    try {
      ...
      const messages: {
        ...
        title: string;
        body: string;
        data: { reminderId: string; stepId?: string, userId?: string, type: string };
      }[] = [];
      ...
      for (const doc of reminderSnapshot.docs) {
        ...
        if (reminder.reminderIntervals && Array.isArray(reminder.reminderIntervals) && !reminder.isStepBased) {
          for (const intervalObj of reminder.reminderIntervals) {
            if (!intervalObj || !Array.isArray(intervalObj.intervals)) continue;
            ...
            for (const interval of intervalObj.intervals) {
              ...
              if ((diffInMinutes === 0 && (intervalObj.currentInterval || 0) < (intervalObj.countOfIntervals || 0))) {
                ...
                console.log(
                  `Sending notification for reminder ${doc.id} at interval ${interval} minutes. Current interval: ${intervalObj.currentInterval}/${intervalObj.countOfIntervals}`
                );
        ...
        if (reminder.isStepBased && reminder.steps && Array.isArray(reminder.steps)) {
          for (const step of reminder.steps) {
            ...
            for (const intervalObj of reminder.reminderIntervals || []) {
              if (!intervalObj || !Array.isArray(intervalObj.intervals) || intervalObj.hasTriggered) continue;
              ...
              for (const interval of intervalObj.intervals) {
                ...
                if (diffInMinutes === 0 && intervalObj.currentInterval < intervalObj.countOfIntervals) {
                  ...
                  console.log(
                    `Sending step notification for reminder ${doc.id} at interval ${interval} minutes. Current interval: ${intervalObj.currentInterval}`
                  );
      ...
      if (messages && messages.length > 0) {
        ...
        try {
          for (const message of messages) {
            ...
            await updateNotificationInteractionRate(message.data.userId, "reminder");
          }
        } catch (error) {
...
export const setQuestionnaireIsReadyFalse = functions.pubsub
  ...
  .onRun(async () => {
    try {
      ...
      const updatePromises = snapshot.docs.map(async (docSnap) => {
        const data = docSnap.data();
        const questionnaireReady = data?.symptomStats?.questionnaireReady;
...
export const updateUserStreaks = functions.pubsub
  ...
  .onRun(async () => {
    ...
    try {
      ...
      const userJogMap: Record<string, { total: number; completed: number }> = {};
      ...
      for (const userId of Object.keys(userJogMap)) {
        ...
        const current = userStats?.jogStats?.currentStreak ?? 0;
        const best = userStats?.jogStats?.bestStreak ?? 0;
        ...
        if (!userData || !userData.expoPushToken || userData.isFocusMode) continue;
      ...
      try {
        for (const msg of notificationQueue) {
          ...
          await updateNotificationInteractionRate(msg.userId, "streak");
        }
      } catch (error) {
        console.error("Error sending notifications:", error);
      }
...
export const sendDueNowReminders = functions.pubsub
  .schedule("every 1 minutes")
  .timeZone("Europe/London")
  .onRun(async () => {
...
export const updateStepCompletionStatus = functions.firestore
  .document("reminders/{reminderId}")
  .onUpdate(async (change, context) => {
    const reminderId = context.params.reminderId;
    const after = change.after.data();
    ...
    // Update each step's completion status
    const updatedSteps = after.steps.map((step) => {
      if (!step.dueDate) return step; // Skip if no dueDate
      ...
      return {
        ...step,
        completed: true,
        completeStatus: stepCompletedOnTime ? "completedOnTime" : "completedLate",
      };
    });
...
export const sendQuestionnaireReminder = functions.pubsub
  .schedule("every 1 minutes")
  .timeZone("Europe/London")
  .onRun(async () => {
    const batch = db.batch();
    ...
    try {
      ...
      for (const doc of userStatsSnapshot.docs) {
        ...
        const questionnaireTime: admin.firestore.Timestamp | null = symptomStats?.questionnaireTime ?? null;
        const questionnaireTimeSet: boolean = symptomStats?.questionnaireTimeSet ?? false;
        ...
        const alreadyCompleted = symptomStats?.dailyLogs?.[todayKey]?.completed === true;
        ...
        try {
          if (userData.isFocusMode) {
            ...
            await updateNotificationInteractionRate(data.userId, "questionnaireReady");
        ...
        batch.set(
          db.collection("userStats").doc(data.userId),
          {
            symptomStats: {
 ...
 */
export const updateNotificationInteractionRate = async (userId: string, type: string): Promise<void> => {
  const userStatsRef = db.collection("userStats").doc(userId);
...
export const sendMorningNotification = functions.pubsub
  ...
  .onRun(async () => {
    ...
    const goodMorningMessages: string[] = [
    ...
    try {
      ...
      for (const message of messages) {
        await sendNotification(message);
        await updateNotificationInteractionRate(message.data.userId, "morningReminder");
...
export const verifyTestUserEmail = functions.https.onCall(async (data) => {
